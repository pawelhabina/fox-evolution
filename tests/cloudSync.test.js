...
test('cloud sync reloads only a newer save written by admin', async () => {
  const { getRemoteChangeAction } = await import('../src/storage/cloudSync.mjs');
  const current = '2026-08-29T09:00:00.000Z';
  const newer = '2026-08-29T09:00:10.000Z';

  assert.equal(getRemoteChangeAction(current, { updatedAt: newer, lastWriterId: 'admin' }, 'client-a'), 'reload');
  assert.equal(getRemoteChangeAction(current, { updatedAt: newer, lastWriterId: 'client-b' }, 'client-a'), 'conflict');
  assert.equal(getRemoteChangeAction(current, { updatedAt: newer, lastWriterId: null }, 'client-a'), 'acknowledge-unattributed');
